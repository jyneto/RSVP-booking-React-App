// import { useEffect, useState } from "react";
// import { getAvailableTables as getAvailFromBookings, submitRsvpWithBooking } from "../api/booking";
// import type { AvailableTable } from "../api/booking";
// // import { submitRSVP } from "../api/guest";
// import { toUtcIso } from "../utils/time";
// import { useBooking } from "../context/BookingCtx";

// export default function SelectTable() {
//   const { state, setState, tables, setTables } = useBooking();
//   const [loading, setLoading] = useState(false);
//   const [submitting, setSubmitting] = useState(false);
//   const [error, setError] = useState<string | null>(null);

//   useEffect(() => {
//     let mounted = true;
//     setLoading(true);
//     const startTimeUtc = toUtcIso(state.date, state.time);
//     getAvailFromBookings(startTimeUtc, state.partySize)

//       .then((rows: AvailableTable[]) => {
//         if (!mounted) return;
//         const bookableTables = rows.filter(r => r.availableSeats >= state.partySize);
//         setTables(bookableTables.map(r => ({
//           id: r.tableId,
//           tableNumber: r.tableNumber,
//           capacity: r.capacity,
//           availableSeats: r.availableSeats,
//         })));
//       })
//       .catch(err => {
//         if (!mounted) return;
//         setError(String(err?.message || err));
//       })
//       .finally(() => {
//         if (!mounted) return;
//         setLoading(false);
//       });
//     return () => { mounted = false; };
//   }, [setTables, state.date, state.time, state.partySize]);
  
//   const selected = tables.find(t => t.id === state.tableId);
//   const canProceed = !!selected && selected.availableSeats >= state.partySize;
//   return (
//     <div>
//       <h2 style={{ color: '#7a8450', textAlign: 'center' }}>Choose table</h2>
//       <div style={{ textAlign: "center", marginBottom: 12 }}>
//         <label style={{ marginRight: 8 }}>
//           <input
//             type="radio"
//             name="partySize"
//             value={1}
//             checked={state.partySize === 1}
//             onChange={() => setState(s => ({ ...s, partySize: 1 }))}
//           />
//           Just me (1)
//         </label>
//         <label>
//           <input
//             type="radio"
//             name="partySize"
//             value={2}
//             checked={state.partySize === 2}
//             onChange={() => setState(s => ({ ...s, partySize: 2 }))}
//           />
//           Me + one (2)
//         </label>
//       </div>
//       {loading && <p style={{ textAlign: 'center' }}>Loading available tables...</p>}
//       {error && <p style={{ color: "crimson", textAlign: 'center' }}>Error: {error}</p>}
//       {!loading && !error && (tables.length === 0 && <p style={{ textAlign: 'center' }}>No available tables for selected date/time/party size.</p>)}
//       <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center", alignItems: "stretch" }}>
//   {tables.map(t => {
//     const canSeat = t.availableSeats >= state.partySize;

//     return (
//       <button
//         key={t.id}
//         className={`table-btn${state.tableId === t.id ? ' selected' : ''}`}
//         disabled={!canSeat}
//         onClick={() => {
//           if (!canSeat) return;
//           setState(s => ({ ...s, tableId: t.id }));
//         }}
//         style={{
//           opacity: canSeat ? 1 : 0.6,
//           cursor: canSeat ? 'pointer' : 'not-allowed'
//         }}
//       >
//         <div><strong>Table #{t.tableNumber}</strong></div>
//         <div>Capacity: {t.capacity}</div>
//         <div
//           style={{
//             color: t.availableSeats > 0 ? '#556B2F' : 'crimson',
//             fontWeight: 600
//           }}
//         >
//           Available seats: {t.availableSeats}
//         </div>
//         {state.tableId === t.id && <div style={{ color: '#7a8450', marginTop: 4 }}>Selected</div>}
//       </button>
//     );
//   })}
// </div>
//       <div style={{ marginTop: 24, display: "flex", gap: 12, justifyContent: 'center' }}>

//         <button
//           // disabled={state == null || submitting}
//           disabled={!canProceed || submitting}
//           onClick={async () => {
//             if (!canProceed) {
//               setError("Selected table has no available seats.");
//               return;
//             }

//             if (state.tableId == null) return;
//             setSubmitting(true);
//             setError(null);
//             try {
//   const startTimeUtcISO = toUtcIso(state.date, state.time);

//   await submitRsvpWithBooking({
//     fullName: state.fullName!,
//     email: state.email!,
//     phone: state.phone || null,
//     allergies: state.allergies || null,
//     tableId: state.tableId!,  
//     startTimeUtcISO,
//     partySize: state.partySize,
//   });

//   setState(s => ({ ...s, step: 4 }));
// } catch (err: any) {
//   const msg =
//     err?.response?.data?.message ||
//     err?.response?.data?.title ||
//     err?.message ||
//     "Could not complete RSVP";
//   setError(String(msg));
// } finally {
//   setSubmitting(false);
// }

//           }}
//           className="wedding-btn"
//         >
//           {submitting ? "Skickar..." : "Fortsätt"}
//         </button>
//         <a href="/" className="wedding-btn">Back to Homepage</a>

//         {error && <p style={{ color: "crimson", textAlign: 'center' }}>{error}</p>}
//       </div>
//     </div>
//   );
// }
import { useEffect, useState } from "react";
import {
  getAvailableTables as getAvailFromBookings,
  submitRsvpWithBooking,
} from "../api/booking";
import type { AvailableTable } from "../api/booking";
import { toUtcIso } from "../utils/time";
import { useBooking } from "../context/BookingCtx";

export default function SelectTable() {
  const { state, setState, tables, setTables } = useBooking();
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let mounted = true;
    setLoading(true);

    const startTimeUtc = toUtcIso(state.date, state.time);

    getAvailFromBookings(startTimeUtc, state.partySize)
      .then((rows: AvailableTable[]) => {
      const bookableTables = rows.filter(r => r.availableSeats >= state.partySize);
        setTables(
          bookableTables.map(r => ({
            id: r.tableId,
            tableNumber: r.tableNumber,
            capacity: r.capacity,
            availableSeats: r.availableSeats,
          }))
        );
      })
      .catch((err) => {
        if (!mounted) return;
        setError(String(err?.message || err));
      })
      .finally(() => {
        if (!mounted) return;
        setLoading(false);
      });

    return () => {
      mounted = false;
    };
  }, [setTables, state.date, state.time, state.partySize]);

  const selected = tables.find((t) => t.id === state.tableId);
  const canProceed =
    !!selected && selected.availableSeats >= state.partySize;

  return (
    <div>
      <h2 style={{ color: "#7a8450", textAlign: "center" }}>
        Choose your table
      </h2>

  
      <div style={{ textAlign: "center", marginBottom: 12 }}>
        <small>
          Party size: <strong>{state.partySize}</strong>
        </small>
      </div>

      {loading && (
        <p style={{ textAlign: "center" }}>
          Loading available tables...
        </p>
      )}
      {error && (
        <p style={{ color: "crimson", textAlign: "center" }}>
          Error: {error}
        </p>
      )}
      {!loading && !error && tables.length === 0 && (
        <p style={{ textAlign: "center" }}>
          No available tables for selected date/time/party size.
        </p>
      )}

      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
          gap: 12,
          justifyContent: "center",
          alignItems: "stretch",
        }}
      >
        {tables.map((t) => {
          const canSeat = t.availableSeats >= state.partySize;

          return (
            <button
              key={t.id}
              className={`table-btn${
                state.tableId === t.id ? " selected" : ""
              }`}
              disabled={!canSeat}
              onClick={() => {
                if (!canSeat) return;
                setState((s) => ({ ...s, tableId: t.id }));
              }}
              style={{
                opacity: canSeat ? 1 : 0.6,
                cursor: canSeat ? "pointer" : "not-allowed",
              }}
            >
              <div>
                <strong>Table #{t.tableNumber}</strong>
              </div>
              <div>Capacity: {t.capacity}</div>
              <div
                style={{
                  color:
                    t.availableSeats > 0 ? "#556B2F" : "crimson",
                  fontWeight: 600,
                }}
              >
                Available seats: {t.availableSeats}
              </div>
              {state.tableId === t.id && (
                <div
                  style={{
                    color: "#7a8450",
                    marginTop: 4,
                  }}
                >
                  Selected
                </div>
              )}
            </button>
          );
        })}
      </div>

      <div
        style={{
          marginTop: 24,
          display: "flex",
          gap: 12,
          justifyContent: "center",
        }}
      >
        <button type="button" className="wedding-btn"onClick={() => setState(s => ({ ...s, step: 1 }))}>Back to Date Selection</button>

        <button
          disabled={!canProceed || submitting}
          onClick={async () => {
            if (!canProceed) {
              setError("Selected table has no available seats.");
              return;
            }

            if (state.tableId == null) return;

            setSubmitting(true);
            setError(null);

            try {
              const startTimeUtcISO = toUtcIso(
                state.date,
                state.time
              );

              await submitRsvpWithBooking({
                fullName: state.fullName!,
                email: state.email!,
                phone: state.phone || null,
                allergies: state.allergies || null,
                tableId: state.tableId!,
                startTimeUtcISO,
                partySize: state.partySize,
              });

              setState((s) => ({ ...s, step: 4 }));
            } catch (err: any) {
              const msg =
                err?.response?.data?.message ||
                err?.response?.data?.title ||
                err?.message ||
                "Could not complete RSVP";
              setError(String(msg));
            } finally {
              setSubmitting(false);
            }
          }}
          className="wedding-btn"
        >
          {submitting ? "Sending..." : "Continue"}
        </button>

        {error && (
          <p style={{ color: "crimson", textAlign: "center" }}>
            {error}
          </p>
        )}
      </div>
    </div>
  );
}
